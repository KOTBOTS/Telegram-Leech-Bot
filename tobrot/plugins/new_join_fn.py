...
async def new_join_f(client, message):
    ...
    if chat_type != "private":
        await message.reply_text(f"<b>This Is Not Allowed To Use Me</b>.\n\nCurrent CHAT ID: <code>{message.chat.id}</code>")
        # leave chat
...
async def welcome_message_f(client, message):
    ...
    await message.reply_text("""**Hey <a href='tg://user?id={update.from_user.id}'>{update.from_user.first_name}</a>!!\n Please Read <a href='https://t.me/Discovery_Updates/7'>This Message</a> To know how to use Me  \n""", disable_web_page_preview=True)
